...
export const osuApi = {
	...
	async getUserRecentScore(userId: string, beatmapId: string) {
		...
		if (token) {
			...
			const recentScore = scores.find((score: any) => {
				return (
					String(score.beatmap.id) === String(beatmapId) &&
					!score.fail &&
	...
	async getStableTopPlayStars(userId: string): Promise<number> {
		...
		try {
			...
			const highAccuracyScores = scores.filter((score: any) => score.accuracy >= 0.95).slice(0, 3);
	...
	async findSuitableBeatmap(
		...
		try {
			...
			const suitableMaps = data.beatmapsets
				.filter(
					(set: any) =>
						set.status === 'ranked' && !recentlySelectedBeatmapsetIds.has(set.id.toString())
				)
				.flatMap((set: any) =>
					set.beatmaps
						.filter(
			...
			const selectedMap =
				suitableMaps[Math.floor(Math.random() * Math.min(suitableMaps.length, 30))];
