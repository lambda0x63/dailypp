...
<div class="min-h-screen bg-dark-300">
	{#if $page.data.user && !isLandingPage}
		<!-- 네비게이션 바 -->
		<nav
			class="fixed top-0 left-0 right-0 z-50 bg-dark-100/95 backdrop-blur-xl border-b border-gray-800/50"
		>
					...
					<div class="hidden md:flex items-center space-x-4">
                     ...
                     {$page.url.pathname === '/stats'
						...
						<div class="relative ml-4">
							...
							>
								<div
									class="relative w-8 h-8 rounded-full bg-dark-200 flex items-center justify-center overflow-hidden"
								>
									<img
                     ...
                     {$page.url.pathname === '/stats' ? 'bg-dark-200 text-white' : 'text-gray-400'}"
						...
						<div class="mt-auto pb-8">
							<div class="flex items-center space-x-3 px-3 py-4">
								<div
									class="relative w-10 h-10 rounded-full bg-dark-200 flex items-center justify-center overflow-hidden"
								>
									<img
										src={`https://a.ppy.sh/${$page.data.user.id}`}
										alt="Avatar"
