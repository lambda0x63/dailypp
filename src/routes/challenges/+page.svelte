...
<div class="min-h-screen bg-gradient-to-b from-dark-200 to-dark-100">
	<!-- 상단 히어로 섹션 -->
	<div
		class="bg-gradient-to-br from-osu-pink/10 via-dark-200 to-dark-100 border-b border-gray-800/50"
	>
			...
			<div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
				<div>
					...
					<p class="mt-3 text-lg text-gray-400 max-w-2xl">
						매일 새로운 비트맵으로 자신의 실력을 시험해보세요. 난이도별 3개의 도전과제가 기다리고
						있습니다.
					</p>
				</div>
						...
						<div class="flex items-center gap-3">
							<div class="flex -space-x-2">
								{#each challenges as challenge}
									<div
										class="w-8 h-8 rounded-full border-2 border-dark-300 flex items-center justify-center
                    {challenge.completed ? 'bg-green-500 text-white' : 'bg-dark-200 text-gray-500'}"
									>
	...
	<div class="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
		...
		{:else}
			...
			<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
				{#each challenges as challenge}
					<ChallengeCard {challenge} onComplete={completeChallenge} {completing} />
				{/each}
			</div>
		{/if}
	</div>
